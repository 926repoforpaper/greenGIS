...
public class TownSpecification {
  public static Specification<Town> searchAll(String search) {
    return new Specification<Town>() {
      @Override
      public Predicate toPredicate(
          Root<Town> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        predicates.add(
            criteriaBuilder.like(
                criteriaBuilder.lower(path.get("code").as(String.class)), stringToFind));
        predicates.add(
            criteriaBuilder.like(
        ...
        predicates.add(
            criteriaBuilder.like(
                criteriaBuilder.lower(
                    root.join("region", JoinType.LEFT).get("id").as(String.class)),
                stringToFind));
        predicates.add(
            criteriaBuilder.like(
                criteriaBuilder.lower(
                    root.join("province", JoinType.LEFT).get("id").as(String.class)),
                stringToFind));
        predicates.add(
            criteriaBuilder.like(
                criteriaBuilder.lower(
                    root.join("municipality", JoinType.LEFT).get("id").as(String.class)),
                stringToFind));
