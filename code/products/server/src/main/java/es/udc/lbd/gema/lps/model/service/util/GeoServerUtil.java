...
public class GeoServerUtil {
  ...
  public void recalculateFeatureTypeBBox(String featureType) throws IllegalArgumentException {
    ...
    String result =
        HTTPUtils.put(url, body, "application/xml", gsProp.getUser(), gsProp.getPassword());

    if (result != null) {
      logger.info(
          "Layer bounding box successfully updated ("
              + gsProp.getWorkspace()
              + ":"
              + featureType
              + ")");
      ...
      logger.error(
          "Error updating layer bounding box (" + gsProp.getWorkspace() + ":" + featureType + ")");
    }
  }
}
