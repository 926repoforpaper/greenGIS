...
public class GeoServerInit implements ApplicationListener<ContextRefreshedEvent> {
  ...
  @Inject private GeoServerProperties gsProp;
  ...
  public void init() {
    ...
    try {
      final GeoServerRESTReader reader =
          new GeoServerRESTReader(gsProp.getUrl(), gsProp.getUser(), gsProp.getPassword());
      final GeoServerRESTPublisher publisher =
          new GeoServerRESTPublisher(gsProp.getUrl(), gsProp.getUser(), gsProp.getPassword());
      final GeoServerRESTStoreManager manager =
          new GeoServerRESTStoreManager(
              new URL(gsProp.getUrl()), gsProp.getUser(), gsProp.getPassword());
      ...
      if (prop.getEnvironment().equals("dev")) {
        ...
        reloadWMSLayersBbox(); // We send a request to the GeoServer to reload the bbox of the WMS
        // layers
      }
  ...
  private void createDataStore(GeoServerRESTStoreManager manager) {
    final GSPostGISDatastoreEncoder storeEncoder =
        new GSPostGISDatastoreEncoder(gsProp.getDatastore());
    ...
    try {
      ...
      for (Object styleObject : styles) {
        ...
        if (styleCached != null && styleCached == true) {
          createdStyles.add(createStyle(publisher, (String) style.get("name")));
        }
      }
    } catch (Exception e) {
      logger.error(e.getMessage(), e);
  ...
  /**
   * It goes through the whole array of layers and associates, to each subLayer, all the styles that
   * will be available. Returns a map whose key is the name of the subLayer and value is a set of
   * available styles of this one.
   *
