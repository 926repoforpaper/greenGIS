<template>
  <v-dialog persistent :value="showStyleDialog" max-width="400px">
    <v-card>
          ...
          <v-container class="mt-3 pb-1">
            ...
            <v-row no-gutters>
              <v-select
                ...
                :disabled="geometryTypeDisabled"
                :items="geoTypes"
                :label="$t('layerManager.wms-style.geometryType')"
                multiple
                :rules="[(v) => !!v || $t('layerManager.geometryTypeRequired')]"
              >
              </v-select>
            </v-row>
...
export default {
  ...
  watch: {
    showStyleDialog(newVal) {
      if (newVal) {
        ...
        this.geometryTypeDisabled = false;
      }
    },
    geometryTypeLayer(newVal) {
      if (this.geoTypes.includes(newVal)) {
        this.geometryType = newVal;
        this.geometryTypeDisabled = true;
      } else {
        this.geometryType = null;
        this.geometryTypeDisabled = false;
      }
  ...
  data() {
    return {
      ...
      strokeOpacity: 1,
      geometryType: null,
      geometryTypeDisabled: false,
      geoTypes: ["point", "polygon", "line"],
    };
  },
  methods: {
    applyNewColor() {
