      ...
      <v-card-text>
        <v-row dense>
          <v-col cols="12" md="6" class="d-flex" style="flex-direction: column">
            ...
            <v-row align="center" dense>
              <v-col cols="3" md="2" class="text-left font-weight-bold">
                {{ $t("t_municipality.prop.population") }}:
              </v-col>
              <v-col cols="9" md="10">
                {{ entity.population }}
              </v-col>
            ...
            <v-row align="center" dense>
              <v-col cols="3" md="2" class="text-left font-weight-bold">
                {{ $t("t_municipality.prop.capital") }}:
              </v-col>
              <v-col cols="9" md="10">
                <router-link
                  :to="{
                    name: 'Town FormDetail',
                    params: { id: entity.capital.id },
                  }"
                  v-if="entity.capital"
                >
                  {{ entity.capital.id }}
          ...
          <v-col cols="12" md="6" class="d-flex" style="flex-direction: column">
            <v-row align="center" dense>
              <v-col cols="3" md="2" class="text-left font-weight-bold">
                {{ $t("t_municipality.prop.extension") }}:
              </v-col>
              <v-col cols="9" md="10">
                <map-field
                  id="extension"
...
const MunicipalityEntityRepository = RepositoryFactory.get(
  "MunicipalityEntityRepository"
);

export default {
  name: "municipalityFormDetail",
  ...
  methods: {
    _fetchData(id) {
      ...
      return MunicipalityEntityRepository.get(id)
