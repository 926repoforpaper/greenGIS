...
const RESOURCE_NAME = "entities/municipalitys";
...
export default {
  ...
  async getGeom(name, cached, options = {}) {
    if (cached) {
      try {
        const response = (await HTTP.get(`${GEOMETRIES_ROUTE}/${name}.json`))
          .data;
        return response || [];
      } catch (err) {
        logger.error("Error getting geometry " + name);
        throw err;
      }
    } else {
      try {
        const response = (
          await HTTP.get(`${RESOURCE_NAME}/geom/${name}`, options)
        ).data;
        return response || [];
      } catch (err) {
